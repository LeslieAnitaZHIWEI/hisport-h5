<template xmlns:v-touch="http://www.w3.org/1999/xhtml">
  <div>
    <div class="ruler-inner">
      <div class="arrow"></div>
      <div v-if="isPC" @click="goLeft" class="leftA">〈</div>
      <div v-if="isPC" @click="goRight" class="rightA">〉</div>
      <div class="number"><strong>{{numberValue}}</strong>{{unit}}</div></div>
...
import {isPC} from '@/utils/index'
  export default {
    props: ["value", "min", "max", "onChange","unit"],
    components: {},
    data: function () {
      return {
        isPC,
    ...
    methods: {
      goLeft(){
        ...
        this.mainValue=this.mainValue+20.8
...
.rightA{
  ...
  .main li {
    background: url(../../assets/ruler.png) left top no-repeat;
